...
var defaultTabata = new Tabata(
    ...
    breakTime: new Duration(seconds: 10));
...
class Settings {
  ...
  Settings(this._prefs) {
    ...
    startBreak = json['startBreak'] ?? 'silent.mp3'; 
    startSet = json['startSet'] ?? 'silent.mp3';
    endWorkout = json['endWorkout'] ?? 'end.mp3';
  }
...
class Workout {
  ...
  _tick(Timer timer) {
    if (_timeLeft == new Duration(seconds: 1)) {
      ...
      if (_timeLeft.inSeconds <= 1 && _timeLeft.inSeconds >= 1) {
        // player.play('pip.mp3');
      }
    }
